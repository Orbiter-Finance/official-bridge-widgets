...
function S() {
  if (x) return A;
  x = 1;
  var l = 1e3, i = l * 60, f = i * 60, r = f * 24, m = r * 7, g = r * 365.25;
  A = function(t, e) {
    e = e || {};
    var n = typeof t;
  ...
  function p(t) {
    var e = Math.abs(t);
    return e >= r ? Math.round(t / r) + "d" : e >= f ? Math.round(t / f) + "h" : e >= i ? Math.round(t / i) + "m" : e >= l ? Math.round(t / l) + "s" : t + "ms";
  }
  function o(t) {
    var e = Math.abs(t);
    return e >= r ? u(t, e, r, "day") : e >= f ? u(t, e, f, "hour") : e >= i ? u(t, e, i, "minute") : e >= l ? u(t, e, l, "second") : t + " ms";
  }
  function u(t, e, n, s) {
    var a = e >= n * 1.5;
...
function B() {
  ...
  M = 1;
  function l(i) {
    r.debug = r, r.default = r, r.coerce = u, r.disable = p, r.enable = g, r.enabled = o, r.humanize = S(), r.destroy = t, Object.keys(i).forEach((e) => {
      r[e] = i[e];
    ...
    function r(e) {
      ...
      function c(...C) {
        if (!c.enabled)
          return;
        const d = c, y = Number(/* @__PURE__ */ new Date()), j = y - (n || y);
        d.diff = j, d.prev = n, d.curr = y, n = y, C[0] = r.coerce(C[0]), typeof C[0] != "string" && C.unshift("%O");
        let w = 0;
        C[0] = C[0].replace(/%([a-zA-Z%])/g, (v, O) => {
          if (v === "%%")
            return "%";
          w++;
          const k = r.formatters[O];
          if (typeof k == "function") {
            const q = C[w];
            v = k.call(d, q), C.splice(w, 1), w--;
          }
          return v;
        }), r.formatArgs.call(d, C), (d.log || r.log).apply(d, C);
      }
      return c.namespace = e, c.useColors = r.useColors(), c.color = r.selectColor(e), c.extend = m, c.destroy = r.destroy, Object.defineProperty(c, "enabled", {
        enumerable: !0,
        configurable: !1,
        get: () => s !== null ? s : (a !== r.namespaces && (a = r.namespaces, h = r.enabled(e)), h),
        set: (C) => {
          s = C;
        }
...
var E;
function L() {
  return E || (E = 1, function(l, i) {
    i.formatArgs = r, i.save = m, i.load = g, i.useColors = f, i.storage = F(), i.destroy = /* @__PURE__ */ (() => {
      let o = !1;
      return () => {
    ...
    function r(o) {
      if (o[0] = (this.useColors ? "%c" : "") + this.namespace + (this.useColors ? " %c" : " ") + o[0] + (this.useColors ? "%c " : " ") + "+" + l.exports.humanize(this.diff), !this.useColors)
        return;
