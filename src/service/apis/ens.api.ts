import { Address, createPublicClient, fallback, http, isAddressEqual } from 'viem'
...
export const resolveName = async (name: string): Promise<ProfileProps | null> => {
  const client = getEnsClient()

  const result = await Promise.all([client.getEnsAddress({ name }), client.getEnsAvatar({ name })]).catch(() => null)
