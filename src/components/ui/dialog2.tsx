...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ...
    id='overlay'
    className={cn(
      'fixed inset-0 h-dvh z-50 backdrop-blur-2xl bg-white/0 backdrop-filter data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
      className
    )}
    {...props}
  />
    ...
    <DialogPortal container={container}>
      ...
      <DialogPrimitive.Content
        ...
        {hideCloseButton ? null : (
          <DialogPrimitive.Close className='absolute right-4 top-4 rounded-full w-10 h-10 shrink-0 flex items-center justify-center bg-muted hover:scale-105 transition-all focus:outline-none disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground'>
...
const DialogHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('flex flex-col text-left px-6 py-6 gap-1', className)} {...props} />
)
...
const DialogTitle = React.forwardRef<React.ElementRef<typeof DialogPrimitive.Title>, React.ComponentPropsWithoutRef<typeof DialogPrimitive.Title>>(
  ({ className, ...props }, ref) => <DialogPrimitive.Title ref={ref} className={cn('text-lg font-heading leading-none ', className)} {...props} />
)
